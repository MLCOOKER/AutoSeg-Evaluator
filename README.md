...
AutoSeg Evaluator's mask rasteriser and all seven geometric metrics
produce bit-for-bit identical output to their upstream reference
implementations on a clinical sample dataset:
...
The full per-ROI per-metric breakdown is in
[`docs/VALIDATION_REPORT.md`](docs/VALIDATION_REPORT.md). The report is
auto-generated by [`scripts/validate_against_upstream.py`](scripts/validate_against_upstream.py)
and contains no PHI (no DICOM UIDs, filenames, patient identifiers,
dates, or institution metadata — only anonymised ROI display names and
voxel/metric values). Anyone can re-run the validator against their
own data to verify the parity claims independently. The equivalence is
also locked in CI via `tests/test_platipy_equivalence.py` and
